  ...
  return new Promise( async (resolve, reject) => {
    try {
      ...
      let info = await transporter.sendMail({
        from: '"Hoang Dung 👻" <hoangdung307198@gmail.com>', // sender address
        to: dataSend.receivedEmail, // list of receivers
        subject: dataSend.language === 'vi'? "Kết quả đặt lịch khám bệnh" : "Medical appointment results", // Subject line
        // text: "Hello world?", // plain text body
        html: getBodyHTMLEnailRemedy(dataSend), // html body,
        attachments: [
          {
...
let getBodyHTMLEnailRemedy = (dataSend) => {
  let result = ''
  if (dataSend.language === 'vi') {
    result = `
        <h3>Xin chào ${dataSend.patientName}</h3>
        <p>Bạn nhận được email này vì đã đặt lịch khám bệnh online trên trang Pikachu thành công</p>
